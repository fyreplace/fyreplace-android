...
fun TextView.setComment(comment: Comment, highlighted: Boolean) {
    setLinkifiedText(if (comment.is_deleted) context.getText(R.string.comment_deleted) else comment.text)
    setTextIsSelectable(!comment.is_deleted)
    alpha = if (comment.is_deleted) 0.6f else 1f
